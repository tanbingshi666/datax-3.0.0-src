...
public class Configuration {
	...
	// root = JSON.parse(json) 本质还是 JSONObject 对象
	...
	/**
	 * 从 JSON 字符串加载 Configuration
	 */
	public static Configuration from(String json) {
		// 如果任务配置文件里面存在 ${} 这种占位符，那么就需要进行填充解析
        json = StrUtil.replaceVariable(json);
		// 检查 json
		checkJSON(json);

		try {
			// 将任务配置文件内容 json 字符串进行封装解析成 Configuration 对象 并返回
	...
	public Configuration merge(final Configuration another,
		...
		// 获取 conf/core.json 文件所有 key 值 也就是递归遍历到叶子节点
		Set<String> keys = another.getKeys();

		for (final String key : keys) {
			// 如果使用更新策略，凡是 another 存在的 key，均需要更新 默认为 false
			if (updateWhenConflict) {
				this.set(key, another.get(key));
				continue;
			}

			// 使用忽略策略，只有 another Configuration 存在但是当前Configuration不存在的key，才需要更新
		...
		// 任务配置文件 + conf/core.json 合并完成 返回 Configuration 对象
		return this;
	}
